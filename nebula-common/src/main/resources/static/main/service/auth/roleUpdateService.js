layui.config({
    ...
    function edit(roleId) {
        $.ajax({
            url: rootUrl + "/uqiansoftAdmin/auth/role/selectRole",
            type: "post",
            cache: false,
            dataType: "json",
            data: {roleId: roleId},
    ...
    //监听提交
    form.on('submit(layuiadmin-app-role-edit)', function (data) {
        var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
        var params = $('#editForm').serializeJSON();
        //提交 Ajax 成功后，关闭当前弹层并重载表格
        $.ajax({
            url: rootUrl + "/uqiansoftAdmin/auth/role/updateRole",
            type: "post",
            cache: false,
            ...
            success: function (result) {
                ...
                layer.msg(result.message, {
                    ...
                    parent.layui.table.reload('LAY-app-role-list'); //重载表格
